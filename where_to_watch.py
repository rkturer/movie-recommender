...
def get_streaming_options(movie_id_tmdb, highest_rated_title):
    ...
    #uses list comprehension to extract the provider names from the lists of dictionaries 
        ...
        streaming_providers = [my_dict["provider_name"] for my_dict in us_results.get("flatrate", [])] or "No streaming options avaliable on Movie Data Base API"
        renting_options = [my_dict["provider_name"] for my_dict in us_results.get("rent", [])] or "No renting options avaliable on Movie Data Base API"
        purchase_options = [my_dict["provider_name"] for my_dict in us_results.get("buy", [])] or "No purchasing options avaliable on Movie Data Base API"
